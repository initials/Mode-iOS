...
- (id) initWithImage:(NSString *)filename
{
    if ((self = [super init])) {
        ...
        ////MWLogDebug(@"SemiSecretTexture initWithImage:%@", filename);
        ...
        if (image == nil) {
            //MWLogDebug(@"SemiSecretTexture initWithImage: could not find image named: %@", filename);
            [self release];
            return nil;
        }
...
- (id) initWithCGColor:(CGColorRef)cgcolor
{
    if ((self = [super init])) {
        ...
        if (componentCount != 4) {
            //MWLogDebug(@"SemiSecretTexture - invalid component count in color...");
            [self release];
            return nil;
        }
...
- (id) initWithSize:(CGSize)Size images:(NSArray *)images locations:(NSDictionary *)locations
{
    if ((self = [super init])) {
        ...
        ////MWLogDebug(@"creating atlas texture: size: (%f,%f)", Size.width, Size.height);
...
- (void) prepareWithWidth:(int)width height:(int)height;
{
    ...
    if (data == NULL) {
        //MWLogDebug(@"SemiSecretTexture prepareWithWidth: calloc failed");
    }
...
- (void) createTexture;
{
    if (textureDepth == SSTextureDepth16Bits5551) {
        ...
        if (data == NULL) {
            //MWLogDebug(@"SemiSecretTexture createTexture: calloc failed (5551)");
        }
        GLubyte r, g, b, a;
        GLshort packed;
        ...
        GLubyte * truncatedData = calloc(paddedSize.width*2,paddedSize.height);
        if (data == NULL) {
            //MWLogDebug(@"SemiSecretTexture createTexture: calloc failed (4444)");
        }
        GLubyte r, g, b, a;
        GLshort packed;
        for (int i = 0; i < paddedSize.width*paddedSize.height*4; i += 4) {
    ...
    if (texture == 0) { //when we reload, no need to generate a new texture
        ...
        if (texture == 0) {
            //MWLogDebug(@"generated texture with 0 for name!");
        }
    }
